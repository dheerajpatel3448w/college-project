...
const HammingDeepDive = ({getbit}) => {
  // --- STATE ---
  const [dataInput, setDataInput] = useState(''); // User Data
  const [cleanCode, setCleanCode] = useState([]);     // Sahi wala code (Reference ke liye)
  const [userCode, setUserCode] = useState([]);       // Wo code jo user manipulate karega (Received)
  const [flippedIndex, setFlippedIndex] = useState(null); // Kaunsa bit user ne toda hai
  const [analysis, setAnalysis] = useState(null);     // Step-by-step details

  // --- HELPER: Generate Hamming Code ---
  ...
  useEffect(() => {
    ...
    setCleanCode(code);
    setUserCode([...code]); // Shuru me userCode bilkul sahi hoga
    setFlippedIndex(null);  // Koi error nahi
    setAnalysis(null);
    setDataInput(getbit); 
         // Result clear
  }, [dataInput,getbit]);

  // --- FEATURE: SINGLE BIT FLIP LOGIC ---
  const handleBitClick = (index) => {
    const newCode = [...cleanCode]; // Hamesha clean copy se start karo (purane errors hat jayenge)
    
    // Agar user ne same bit pe click kiya jo already flipped hai -> To use wapas theek kar do
    if (flippedIndex === index) {
      setFlippedIndex(null);
      setUserCode(newCode); // Bilkul clean wapas
    } else {
      // Naya bit flip karo
    ...
    setAnalysis(null); // Jaise hi change ho, purana result hata do
  };

  // --- LOGIC: DETAILED ANALYSIS (The Core Request) ---
  const analyzeError = () => {
    const bits = userCode;
    ...
    // Check Parity Bits Step-by-Step
    for (let i = 0; i < r; i++) {
      const pPos = Math.pow(2, i); // 1, 2, 4...
      let checkedIndices = [];
      let sum = 0;

      // Find which bits cover this parity
      for (let k = 1; k <= totalLen; k++) {
        if ((k & pPos) !== 0) {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-300 p-6 font-sans flex flex-col items-center">
      <div className="w-full max-w-4xl space-y-6">
            ...
            {userCode.map((bit, idx) => {
              ...
              return (
                <div key={idx} className="flex flex-col items-center group cursor-pointer" onClick={() => handleBitClick(idx)}>
                   ...
                   `}>
                      {bit}
                      {/* Original value hint on hover if flipped */}
                      {isFlipped && (
        ...
        {/* DETAILED MATH LOGS (Step-by-Step) */}
        {analysis && (
