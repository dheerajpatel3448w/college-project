...
const DetailedHammingGenerator = ({getbit,setgetbit}) => {
  ...
  const [steps, setSteps] = useState([]); // To store detailed logs
  ...
  const calculateHamming = (bits) => {
    ...
    // Step 1: Calculate Redundant Bits (r)
    ...
    let logSteps = []; // Temporary array to store logic
    ...
    // Step 2: Place Data and Parity Placeholders
    let j = 0;
    for (let i = 1; i <= totalLen; i++) {
      if ((i & (i - 1)) === 0) {
        arr[i - 1] = { type: 'P', val: 0, pos: i, isParity: true }; // Placeholder 0
      } else {
    ...
    // Step 3: Calculate Parity Values
    ...
    for (let i = 0; i < r; i++) {
      ...
      for (let k = 1; k <= totalLen; k++) {
        // Check if bit 'k' contributes to parity 'pPos'
        // Logic: Bitwise AND
        if ((k & pPos) !== 0) {
           // We don't include the parity bit itself in the sum for calculation (conceptually)
           if (k !== pPos) {
             count += arr[k - 1].val;
      ...
      <div className="w-full max-w-5xl space-y-8">
        ...
        <div className="text-center border-b border-slate-800 pb-6">
            ...
            <h1 className="text-3xl font-bold text-green-500 mb-2 flex items-center justify-center gap-3">
                <Binary className="w-8 h-8"/> Hamming Code Detailer
            </h1>
            ...
            {steps.map((step, index) => (
                <div key={index} className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
                    ...
                    <div className="p-6">
                        ...
                        {/* Special Rendering for Parity Calculations */}
                        {step.subSteps && (
